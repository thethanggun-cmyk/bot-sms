...
def start(message):
    ...
    markup.add("💳 Nạp Tiền")
    markup.add("Kiếm Tiền Free Để Lấy Otp 🆓")
    if int(uid) == ADMIN_ID: markup.add("⚙️ Quản Lý Admin")
    bot.send_message(message.chat.id, text, reply_markup=markup, parse_mode="Markdown")

# --- [XỬ LÝ KIẾM TIỀN FREE - CẬP NHẬT MỚI] ---
@bot.message_handler(func=lambda m: m.text == "Kiếm Tiền Free Để Lấy Otp 🆓")
def free_money(m):
    bot.reply_to(m, "⚠️ **Chức năng này sẽ có sau ít ngày ae chờ nhé**", parse_mode="Markdown")
...
def process_deposit_step(message):
    if message.text in ["💳 Nạp Tiền", "🛒 Mua Dịch Vụ", "👤 Thông Tin", "/start", "⚙️ Quản Lý Admin", "Kiếm Tiền Free Để Lấy Otp 🆓"]:
    ...
    try:
        ...
        qr_url = f"https://img.vietqr.io/image/{bank}-{stk}-compact2.jpg?amount={amt}&addInfo={nd}&accountName=Gun%20Store"
        ...
        caption = (
            ...
            f"📝 Nội dung: `{nd}`\n"
            "━━━━━━━━━━━━━━━━━━\n"
            "⚠️ *Vui lòng chuyển đúng nội dung để được duyệt nhanh nhất!*"
        )
...
def conf_dep(c):
    ...
    bot.answer_callback_query(c.id, "Đã gửi yêu cầu tới Admin!")
    mk = types.InlineKeyboardMarkup()
